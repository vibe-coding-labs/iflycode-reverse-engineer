...
**文件大小**: 3605 行
**数据结构**: JSON 数组，每个元素为 `{"extension": "ext", "value": "LanguageName"}`
...
| Visual Basic | 8 |
| PHP | 8 |
| Common Lisp | 4 (另有 8 扩展名在 lang->ext 映射) |
| Awk | 5 |
| COBOL | 5 |
...
**文件大小**: 3376 行
**数据结构**: JSON 数组，每个元素为 `{"name": "LanguageName", "type": "type", "extensions": [".ext1", ".ext2"]}`
...
| 对比项 | 值 |
|--------|-----|
| fileExtensionLanguageMappings 中的扩展名 | 901 |
| languageFileExtensionMappings 中的扩展名 | 900 |
| 两者共有的扩展名 | 900 |
| 仅在 ext->lang 中存在的扩展名 | 1 (.csharp) |
| 仅在 lang->ext 中存在的扩展名 | 0 |

两个映射文件本质上是同一数据集的双向视图：
- `fileExtensionLanguageMappings` 以扩展名为键，查找对应语言
- `languageFileExtensionMappings` 以语言名为键，查找对应扩展名列表
...
LanguageInfoManager / CodeLanguageInfoSupport
  |-- fileExtensionLanguageMappings.json (扩展名 -> 语言)
  |-- languageFileExtensionMappings.json (语言 -> 扩展名)
...
1. **语言映射覆盖广泛**：支持 371 种语言、901 个文件扩展名，覆盖主流编程语言和大量小众语言。映射数据来源于 GitHub Linguist 规范。
...
4. **属性文件集中管理 UI 文本**：所有中文 UI 文本通过 Unicode 转义存储在 properties 文件中，`BasicActionsBundle` 是主要的消息包，覆盖批量单测、内联聊天、知识库等全部功能模块。

5. **配置端点暴露**：`aicode.otel.endpoint` 暴露了 OpenTelemetry 追踪端点 `https://iflycode-xfsaas.xfyun.cn/v1/traces`，`aicode.complete.time.out` 设置补全超时为 10 秒。

6. **构建信息**：插件版本 3.4.2-222，基于 IntelliJ IC-2022.2 SDK 构建，使用 Gradle 7.6.1 + IntelliJ Plugin 1.13.3。
